  ...
  const handleStatusChange = (newStatus: SpokeStatus) => {
    ...
    if (newStatus === 'Partial Docs' && !canEditAsApprover) {
        ...
        toast({
            ...
            description: `${dealer.name}'s status has been set to 'Awaiting Docs Approval' for review.`,
        });
    } else {
        toast({
            title: 'Dealer Status Updated',
            description: `${dealer.name}'s status is now ${finalStatus}.`,
        });
    }
    updateDealer({ id: dealer.id, status: finalStatus });
  };
